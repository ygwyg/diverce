...
function safelyUpdateConversionStatus(
  projectId: string,
...
async function startConversion(projectId: string, options: any) {
  try {
    ...
    if (!hasGitRepository && hasLinkRepository && project.link) {
      ...
      global.conversionStore[projectId].logs.push(
        `Detected Git repository from link field: ${project.link.org}/${project.link.repo}`
      );
    }
    ...
    // Update status
    global.conversionStore[projectId].logs.push(
      `Fetching project details for ${project.name}...`
    );
    global.conversionStore[projectId].logs.push(
      `Repository: ${project.gitRepository.repo}`
    );
    ...
    if (
      project.gitRepository.type === "github" &&
      ...
      global.conversionStore[projectId].logs.push(
        `Using formatted GitHub URL: ${formattedRepoUrl}`
      );
    }

    global.conversionStore[projectId].logs.push(
      `Cloning repository from ${formattedRepoUrl}...`
    );
    ...
    if (options.createBranch && options.branchName) {
      global.conversionStore[projectId].logs.push(
        `Creating branch: ${options.branchName}...`
      );
      const branchCreated = await createBranch(projectPath, options.branchName);

      if (!branchCreated) {
        global.conversionStore[projectId].logs.push(
          `Warning: Failed to create branch ${options.branchName}. Continuing on current branch.`
        );
      } else {
        global.conversionStore[projectId].logs.push(
          `Created branch: ${options.branchName}`
        );
      }
    }
    ...
    if (result.success && options.commitAndPush) {
      ...
      const commitResult = await commitAndPush(
        projectPath,
        "Convert to @opennextjs/cloudflare"
      );
      ...
      if (commitResult) {
        ...
        global.conversionStore[projectId].logs.push(
          "Warning: Failed to commit and push changes"
        );
      }
    }

    // Update final status
    global.conversionStore[projectId].status = result.success
      ? "success"
      : "failed";
