...
export class ConversionPipeline {
  ...
  constructor(options: ConversionOptions) {
    ...
    // Calculate the full app path, defaulting to projectPath if no appDirectory is provided
    this.appPath = options.appDirectory
      ? path.join(options.projectPath, options.appDirectory)
      : options.projectPath;
  }
  ...
  private async verifyNextJsProject() {
    ...
    if (!packageJson.dependencies?.next && !packageJson.devDependencies?.next) {
      throw new Error(
        "This project does not appear to be a Next.js project (next package not found)"
      );
    }
    ...
    try {
      const { stdout } = await execAsync(
        'grep -r "export const runtime = \\"edge\\"" --include="*.js" --include="*.jsx" --include="*.ts" --include="*.tsx" .',
        { cwd: this.appPath }
      );

      if (stdout.trim()) {
        this.log(
          "⚠️ WARNING: Edge Runtime usage detected. These will be removed during conversion."
        );
      } else {
  ...
  private async installDependencies() {
    ...
    try {
      // Default to npm if packageManager is not specified
      const packageManager = this.options.packageManager || "npm";

      // Create the appropriate command based on the package manager
      const installCommand =
        packageManager === "npm"
          ? "npm install --save-dev @opennextjs/cloudflare@latest wrangler@latest"
          : "pnpm add --save-dev @opennextjs/cloudflare@latest wrangler@latest";
  ...
  private async createOpenNextConfig() {
    ...
    const configPath = path.join(this.appPath, "open-next.config.ts");

    const config = `import { defineCloudflareConfig } from "@opennextjs/cloudflare";
${
  this.options.enableKVCache
...
`;
  ...
  private async createWranglerConfig() {
    ...
    const wranglerPath = path.join(this.appPath, "wrangler.jsonc");
    const kvNamespaces =
      this.options.enableKVCache && this.options.kvNamespaceId
        ? `[
  {
...
`;
  ...
  private async removeConflictingReferences() {
    ...
    try {
      await execAsync(
        "find . -type f -name '*.js' -o -name '*.jsx' -o -name '*.ts' -o -name '*.tsx' | xargs -I{} sed -i '' 's/export const runtime = .edge.;/\\/\\/ Removed edge runtime declaration/g' {}",
        {
          cwd: this.appPath,
        }
      );
      ...
      this.log(
        "Note: Could not automatically remove edge runtime declarations. You may need to remove these manually."
      );
    }
  ...
  private async updateGitignore() {
    ...
    const gitignorePath = path.join(this.appPath, ".gitignore");
    let content = "";
